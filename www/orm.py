...
class TextField(Field):       
    def __init__(self, name=None, primary_key=False, default=""):
        super().__init__(name, 'text', priamry_key, default )
...
class ModelMetaclass(type):        
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__insert__'] = 'insert into `%s` (%s, `%s`) values (%s)' % (tableName, ', '.join(escaped_fields), primaryKey, caeate_args_string(len(escaped_fields) + 1) )
...
class Model(dict, metaclass=ModelMetaclass):    
    ...
    @classmethod
    async def findNumber(cls, selectField, where, args=None):    
